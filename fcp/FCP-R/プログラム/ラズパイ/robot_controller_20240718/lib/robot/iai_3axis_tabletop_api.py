...
class RobotApi():   
    ...
    def getFlagStatus(self, flag_no: int) -> bool:
        """ グローバルフラグ読み込み
        """
        return (self.getFlagStatusBase(flag_no))
    ...
    def getUnitStatusBase(self):
        ...
        # R  
        if (MAX_AXIS >=4):  
            # self.moving_axis[Rz]     = True if self.splitBytesBase(int(received_data_str[19], 16), 0, 1) == 1 else False  # move_status 
            self.origin_axis[Rz]     = True if self.splitBytesBase(int(received_data_str[19], 16), 2, 1) == 1 else False  # home_high 
            self.servo_axis[Rz]      = True if self.splitBytesBase(int(received_data_str[19], 16), 3, 1) == 1 else False  # servo_status 
            # self.cmd_result_axis[Rz] = True if self.splitBytesBase(int(received_data_str[19], 16), 4, 1) == 1 else False  # cmd_result           
    ...
    # P22:フラグ照会
    def getFlagStatusBase(self, flag_no):
        # フラグNo.を指定の数値に分解
        base = int(flag_no / 100) * 100
        number = flag_no
        if number == base:
            base_flag_no = base
            bit_no = 0
        input_val = number
        input_offset = input_val - base
        if input_offset == 0:
            base_flag_no = base
            bit_no = 0
        elif input_offset % 8 == 0 and input_offset > 0:
            base_flag_no = input_val
            bit_no = 0
        else:
            quotient = input_offset // 8
            remainder = input_offset % 8
            base_flag_no = base + (quotient * 8)
            bit_no = remainder
        ...
        cmd_data_string.append(self.convBytes2StrBase(base_flag_no, 4))   # 照会開始フラグNo.
        cmd_data_string.append('0008')                                    # 照会フラグ数(0008=8bit)
        high_SC, low_SC = self.calcSC(''.join(cmd_data_string))           # SC計算
        cmd_data_string.append(high_SC+low_SC)                            # SC
        cmd_data_string.append('\r')                                      # CR
        cmd_data_string.append('\n')                                      # LF
        ...
        # ビットデータ抽出
        # 16進数文字列を10進数に変換
        decimal_val = int(received_data_str[6], 16)        
        # ビットマスクを作成
        bitmask = 1 << bit_no
        # 対象ビットが1か0かを判定
        return (decimal_val & bitmask) != 0
...
if __name__ == '__main__':
    ...
    # node.setGlobalFlagOffBase(600)

    node.setGlobalFlagOffBase(615)
    # node.setGlobalFlagOffBase(600)
    # node.setOutputONBase(316)
    print(node.getFlagStatusBase(615))
