...
class RobotApi():   
    ...
    def getUnitStatusBase(self):
        ...
        if (MAX_AXIS >=4): self.current_pos[R] = self.convStr2DecBase(received_data_str[23]) 
        ##############################################################
        # 軸ステータス
        ############################################################## 
        # X
        ...
        # R  
        if (MAX_AXIS >=4):  
            # self.moving_axis[R]     = True if self.splitBytesBase(int(received_data_str[19], 16), 0, 1) == 1 else False  # move_status 
            self.origin_axis[R]     = True if self.splitBytesBase(int(received_data_str[19], 16), 2, 1) == 1 else False  # home_high 
            self.servo_axis[R]      = True if self.splitBytesBase(int(received_data_str[19], 16), 3, 1) == 1 else False  # servo_status 
            # self.cmd_result_axis[R] = True if self.splitBytesBase(int(received_data_str[19], 16), 4, 1) == 1 else False  # cmd_result           
    ...
    # P47:フラグ状態変更
    def setFlagStatusBase(self, flag_no, on_off_flag):
        if (on_off_flag == True):
            on_off_flag = '1'
        else:
            on_off_flag = '0'
