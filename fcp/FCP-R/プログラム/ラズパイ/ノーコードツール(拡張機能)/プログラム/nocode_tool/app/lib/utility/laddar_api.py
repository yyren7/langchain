# -*- coding : UTF-8 -*-
import time
from lib.utility.globals import R, MR, CR, LR, T, X, Y, Z, RX, RY, RZ, VEL, ACC, DEC, DIST, WAIT

class LaddarSeqAPI():   
    def __init__(self, max_R_relay: int, max_MR_relay: int, max_EM_relay: int, max_LR_relay: int, max_T_relay: int, max_C_relay: int): 
        ##############################################################
        # クラス変数定義
        ##############################################################  
        # シーケンス作成用
    ...
    def LDP(self, relay_name: str, relay_no: int = 0, offset: int = 0):
        self.ldlg = (self.ldlg << 1) | (self.aax << 1) 
        self.aax = 0x0 
        if((self.getRelay(relay_name, relay_no)) and not(self.getRelay('PREV_'+relay_name, relay_no))):
            self.aax |= 0x01 
    ...
    def TIM(self, relay_name: str, relay_no :int, wait_msec: int): # wait_time:×1msec
        if (self.aax & self.iix):  
            if (not(self.getRelay(relay_name, relay_no))):
            ...
            self.resetRelay(relay_name, relay_no)
            # print('++++++++++++++++++++++ reset +++++++++++++++++++++++++')
