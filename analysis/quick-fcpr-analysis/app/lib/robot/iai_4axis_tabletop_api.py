...
class RobotApi():   
    def __init__(self, dst_ip_address: str, dst_port: str, dev_port: str, baurate: int):  
        ...
        # これ以降IAI卓ロボ専用   
        self.moving_axis     :List[float] = [False] * 6
        self.origin_axis     :List[float] = [False] * 6
        self.servo_axis      :List[float] = [False] * 6
        self.cmd_result_axis :List[float] = [False] * 6      
        ##############################################################
        # ロボット接続開始
        ##############################################################  
    ...
    def waitArrive(self, target_pos: List[float], width: float) -> None:
        ...
        # Arrived確認 
        ###############################################################
        target_pos[0] = target_pos[0] - 250
        target_pos[1] = target_pos[1] + 210
        # 現在位置と目標位置の差分取得
    ...
    def printRobotStatus(self) -> None:
        ...
        print('R  = {0:.3f}'.format(self.current_pos[Rx]))
    ...
    def getUnitStatusBase(self):
        ...
        if (MAX_AXIS >=4): self.current_pos[Rx] = self.convStr2DecBase(received_data_str[23])
        ##############################################################
        # 軸ステータス
        ############################################################## 
        # X
        ...
        # R
        if (MAX_AXIS >=4):  
            # self.moving_axis[Rx]     = True if self.getBytesBase(int(received_data_str[19], 16), 0, 1) == 1 else False  # move_status 
            self.origin_axis[Rx]     = True if self.getBytesBase(int(received_data_str[19], 16), 2, 1) == 1 else False  # home_high 
            self.servo_axis[Rx]      = True if self.getBytesBase(int(received_data_str[19], 16), 3, 1) == 1 else False  # servo_status 
            # self.cmd_result_axis[Rx] = True if self.getBytesBase(int(received_data_str[19], 16), 4, 1) == 1 else False  # cmd_result           
