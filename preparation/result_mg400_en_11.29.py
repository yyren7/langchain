...
def main():
    ...
    while True:
        for k in range(len(pick_points)):
            ...
            move_to_position(move_client, float(pick_point['x']), float(pick_point['y']), float(pick_point['z']) + 40,
                             float(pick_point['r']))

            # Move down 40 cm
            move_to_position(move_client, float(pick_point['x']), float(pick_point['y']), float(pick_point['z']),
                             float(pick_point['r']))
            ...
            move_to_position(move_client, float(place_point['x']), float(place_point['y']),
                             float(place_point['z']) + 40, float(place_point['r']))

            # Move down 40 cm
            move_to_position(move_client, float(place_point['x']), float(place_point['y']), float(place_point['z']),
                             float(place_point['r']))
