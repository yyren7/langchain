...
def main():
    ...
    while True:
        ...
        # 1. 到达抓取开始坐标正上方20cm
        move_p.MovJ(pos_object[k % len(pos_object)][0], pos_object[k % len(pos_object)][1],
                    pos_object[k % len(pos_object)][2] + 20, pos_object[k % len(pos_object)][3])
        time.sleep(1)

        # 2. 向正方向下移20cm
        move_p.MovL(pos_object[k % len(pos_object)][0], pos_object[k % len(pos_object)][1],
                    pos_object[k % len(pos_object)][2], pos_object[k % len(pos_object)][3])
        time.sleep(1)
